...
public class DeleteUserHandler : IRequestHandler<DeleteUserCommand, DeleteUserResult>
{
    ...
    public async Task<DeleteUserResult> Handle(DeleteUserCommand request, CancellationToken cancellationToken)
    {
        ...
        var existingUserWithId = await _userRepository.GetByIdAsync(request.Id, cancellationToken);

        if (existingUserWithId == null)
            throw new InvalidOperationException($"User with id {request.Id} not exists");

        var deletedUser = await _userRepository.DeleteAsync(existingUserWithId, cancellationToken);
